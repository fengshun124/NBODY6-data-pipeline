...
```mermaid
flowchart TD
    A["<b>Raw NBODY6 Files</b><br/>OUT34, OUT9, fort.82, fort.83, fort.19, densCentre.txt"]
    B["<b>Ingestion and Synchronisation</b><br/>NBODY6DataLoader + SnapshotAssembler<br/><i>Parse and merge raw outputs into coherent snapshots</i>"]
    C["<b>Intrinsic Snapshot Collection</b><br/>Snapshot / SnapshotSeries"]
    D["<b>Observation-space Transformation</b><br/>PseudoObserver<br/><i>Apply projection and unresolved-source blending</i>"]
    E["<b>Synthetic Catalogue Collection</b><br/>PseudoObservedSnapshot / SnapshotSeriesCollection"]

    A --> B --> C --> D --> E

    classDef raw fill:#f9d6d5,stroke:#333,stroke-width:2px,color:#111;
...
- `MPL_STYLE`: optional matplotlib style
...
- Parse and assemble raw NBODY6 outputs. This step builds `SnapshotSeries`, caches results with `joblib`, and exports overall and annular statistics.
...
- Compute inclination statistics to produce per-snapshot inclination summaries.
  ...
  **Options**
  - `--log-file PATH`: custom log file path. Default is `OUTPUT_BASE/log/batch_collect_inclination.log`.

#### Notes

- Both scripts read raw data from `SIM_ROOT_BASE` and write outputs under `OUTPUT_BASE`, as configured in `.env`.
- Core counts and parallelism can be adjusted in the scripts.
- Log files are written to `OUTPUT_BASE/log/` by default; use `--log-file` to redirect output to a custom path.

### Outputs

The pipeline writes results under `OUTPUT_BASE` using this structure.
...
The data processing pipeline has two main stages.
...
In `PseudoObservedSnapshot`, the `stars` DataFrame may contain merged photocentric objects that represent unresolved binaries.

**Key metrics** use physical units of pc, km/s, $\mathrm{M}_{\odot}$, and Myr.
...
Two related "within tidal radius" flags are provided. `is_within_r_tidal` means within `r_tidal`. `is_within_2x_r_tidal` means within $2r_{\mathrm{tidal}}$.

_All radial quantities and tidal radii are measured relative to the density center recalculation provided in `densCentre.txt`, which serves as the primary inertial reference._

Binary systems are classified with the criteria below.
...
A `SnapshotSeries` is a temporal sequence of `Snapshot` objects that stores one complete simulation. [`SnapshotSeriesCollection`](./src/nbody6/data/collection.py) holds multiple `SnapshotSeries`, such as the same run pseudo-observed from different positions.
...
|   **`fort.83`**    | Stellar properties for **single stars** and **unregularized binaries**. This includes mass, radius, bolometric luminosity, and temperature.                                                    | `name`, `mass` $[\mathrm{M}_{\odot}]$, `rad` $[\log_{10}(\mathrm{R}_{\odot})]$, `zlum` $[\log_{10}(\mathrm{L}_{\odot})]$, `tempe` $[\log_{10}(\mathrm{K})]$                                                   | `time`                                                                                                                        |
|   **`fort.19`**    | Pairing information for **unregularized binaries**. Binary components (`name1` / `name2`) may reference regularized binary centers (`cmName`).                                                 | `name1`, `name2`, `ecc`, `p` $\log_{10}(P/\mathrm{day})$                                                                                                                                                      | `time`                                                                                                                        |
...
|     **`OUT9`**     | Pairing information and orbital attributes of **regularized binaries**. Each row links two component stars (`name1`, `name2`) to their common center of mass (`cmName`).                       | `cmName`, `name1`, `name2`, `ecc`, `p` $\log_{10}(P/\mathrm{day})$                                                                                                                                            | `time`                                                                                                                        |
...
- **Position and velocity merging.**
  **`OUT34`** and **`OUT9`** data are combined to expand regularized binary centers of mass into individual component positions.

- **Attribute integration.**
  Stellar properties from `fort.82` and `fort.83` are merged with the positional data.

- **Distance calculations.**
  All distance-based metrics are computed relative to the density center from `densCentre.txt`.

- **Binary pairing.**
  Binary pairings from both `OUT9` (regularized) and `fort.19` (unregularized) are integrated into a unified catalog.
...
The pipeline handles hierarchical systems, such as triples or quadruples, recursively.
Innermost binaries are merged first. The resulting "pseudo-stars" are then merged with outer components until the entire unresolved system is represented by a single entry. Some examples of these merges are presented below:
...
- **Unresolved binary pair plus one resolved star**
  The binary pair is merged first, then combined with the other component if it is also unresolved.
...
Given the properties of two objects, the merged values are:
...
- **Total mass.** $M_{\mathrm{total}} = M_1 + M_2$
...
For hierarchical systems, these formulas are applied recursively at each merging step based on the luminosity-weighted centers.
...
NBODY6 produces snapshots at adaptive timestamps that differ between runs, making direct comparison impossible without interpolation. For example:

- **Run A** may produce snapshots at: 0, 1.0, 2.0, 3.0, ... Myr
- **Run B** may produce snapshots at: 0, 0.8, 1.6, 2.4, ... Myr

The alignment procedure applied in the notebooks is:

  1. **Pre-aggregation.** Before alignment, all loaded CSV files are concatenated and averaged per timestamp within each unique combination of the full identifying key: observer position `galactic_x`, radial bin `radius`, and the initial-condition attributes `init_gc_radius`, `init_metallicity`, `init_mass_lv`, and `init_pos`. This reduces data volume prior to interpolation.

  2. **Uniform grid construction.** A shared integer grid is built from 0 Myr to the maximum observed timestamp in 1 Myr steps using `numpy.arange`.

  3. **Per-group interpolation.** For each unique combination of the grouping keys, the native snapshot timestamps are unioned with the uniform grid. The per-group metrics are then reindexed onto this combined index, and gaps are filled by time-proportional linear interpolation (`interpolate("index")`). Finally, the result is reindexed back onto the 1 Myr grid alone, discarding the intermediate float-timestamp rows.

  4. **No extrapolation.** The `limit_area="inside"` constraint ensures interpolation only fills gaps strictly between the first and last snapshot of each group. Timestamps beyond a run's end remain `NaN` and are dropped subsequently.

  5. **Aggregation across observer positions.** Once aligned, the data are grouped by all keys except `init_pos` and aggregated with mean, standard deviation, and median across the `init_pos` variants for each radial bin and timestamp.

The aligned and aggregated data are cached as `aligned_annular_stats_r_hm.parquet` and `aligned_filtered_annular_stats_r_hm.parquet` in the `OUTPUT_BASE/cache/stats/` directory, ensuring that the interpolation step is performed only once.

## :construction: Future Work

Extend `PseudoObserver` with a synthetic photometry step so pseudo-observed snapshots can be filtered and compared in a survey-like workflow, such as Gaia-like selections. Conceptually, this involves:
